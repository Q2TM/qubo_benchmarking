...
for i in range(0,m):
    for j in range(0,m):
		    ...
		    if result == True:
				    hamiltonian_matrix[i][j] = hamiltonian_matrix[i][j] - h_list[position]

for i in range(0,m):
    for j in range(0,n-1):
		    for k in range(j+1,n):
				    hamiltonian_matrix[i][i] = hamiltonian_matrix[i][i] - J_list[j][k] * ((-1)**(int(spin_chains[i][j])*int(spin_chains[i][k])))
